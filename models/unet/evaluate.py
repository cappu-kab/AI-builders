...
def _score_wer_cer(ref: str, hyp: str,
    ...
    Prefix matching
    ---------------
    When the reference label covers more audio than the evaluated segment,
    full-reference WER unfairly penalises correct transcriptions of the prefix.
    This function tries both the full reference and a prefix of length ≈
    segment_seconds × words_per_second, returning the lower WER.

    WER: tokenised with ``_tok_thai`` (pythainlp when available).
    CER: character edit-distance on the raw stripped strings (always full ref).
    """
    ...
    if len(ref_toks) > n_prefix and hyp_toks:
        wer = min(wer_full, _wer_tokens(ref_toks[:n_prefix], hyp_toks))
    else:
        wer = wer_full
