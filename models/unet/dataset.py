...
# ---------------------------------------------------------------------------
# Low-frequency / high-frequency noise generators (70/30 split support)
# ---------------------------------------------------------------------------
...
def generate_lf_noise(length: int, sample_rate: int = 16_000) -> np.ndarray:
    """Synthetic LF noise (<200 Hz dominant): HVAC rumble, fan, 50/60 Hz hum."""
    ...
    alpha = random.uniform(-1.8, -2.5)          # brown-to-deeper power-law
    color = freqs_safe ** alpha
    lp = (1.0 / (1.0 + np.exp(np.clip((freqs - 200.0) / 25.0, -500.0, 500.0)))).astype(np.float32)
    X = X * color * lp
    # Tonal hum at 50/60 Hz + harmonics (40% chance)
...
def generate_click_noise(length: int, sample_rate: int = 16_000) -> np.ndarray:
    ...
    for _ in range(n_clicks):
        ...
        click_len = random.randint(1, max(1, int(0.003 * sample_rate)))  # ≤3 ms
...
def generate_hiss_noise(length: int, sample_rate: int = 16_000) -> np.ndarray:
    """High-frequency hiss: wideband noise emphasising content above 1.5–4 kHz."""
    white = np.random.randn(length).astype(np.float32)
    X = np.fft.rfft(white)
    freqs = _rfftfreq(length, d=1.0 / sample_rate)
    cutoff = random.uniform(1500.0, 4000.0)
    gain = (1.0 / (1.0 + np.exp(-(freqs - cutoff) / 250.0))).astype(np.float32)
...
def generate_burst_noise(length: int, sample_rate: int = 16_000) -> np.ndarray:
    ...
    y = np.zeros(length, dtype=np.float32)
    n_bursts = random.randint(1, 5)
    for _ in range(n_bursts):
        start = random.randint(0, max(0, length - 1))
        dur = random.randint(int(0.01 * sample_rate), int(0.10 * sample_rate))  # 10–100 ms
...
class SpeechDenoiseDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 noise_pool_max: int = 0,
                 preload_clean: bool = False,
                 clean_pool_max: int = 6000,
        ...
        # 70/30 split: fraction of training samples that get LF noise (<lf_cutoff_hz)
        self.lf_noise_ratio = float(lf_noise_ratio)
        self.lf_cutoff_hz = float(lf_cutoff_hz)
        self._noise_pool_max = int(noise_pool_max)  # 0 = load all
        ...
        # Pre-load noise pool into RAM
        # noise_pool_max > 0 caps the number of files to avoid RAM pressure.
        # Rule of thumb: 4 000 files ≈ 1 GB; 16 000 ≈ 4 GB; 0 = all (≈ 8 GB).
        self.noise_pool: List[np.ndarray] = []
        if preload_noise:
            files_to_load = self.noise_files
            if self._noise_pool_max and self._noise_pool_max < len(files_to_load):
                rng_state = random.getstate()
                random.seed(42)
                files_to_load = random.sample(files_to_load, self._noise_pool_max)
                random.setstate(rng_state)
                print(f"[dataset] noise pool capped to {self._noise_pool_max} / "
                      f"{len(self.noise_files)} files (~"
                      f"{self._noise_pool_max * 251 // 1024} MB)")
            for p in files_to_load:
                try:
                    n = np.load(p, mmap_mode="r").astype(np.float32).squeeze()
                    if n.ndim > 1:
                        n = n.mean(axis=0)
                    n = n - n.mean()
                    if n.size > 0:
        ...
        # Pre-load clean pool into RAM (training only — val/test always loads by index).
        # Eliminates disk I/O for clean files which is the dominant bottleneck on Windows.
        # Rule of thumb: 6000 files × ~192 KB ≈ 1.1 GB RAM extra.
        self.clean_pool: List[np.ndarray] = []
        if preload_clean and split == "train":
            files_to_load = self.clean_files
            if clean_pool_max and clean_pool_max < len(files_to_load):
                rng_state2 = random.getstate()
                random.seed(44)
                files_to_load = random.sample(files_to_load, clean_pool_max)
                random.setstate(rng_state2)
                print(f"[dataset] clean pool capped to {clean_pool_max} / "
                      f"{len(self.clean_files)} files (~"
                      f"{clean_pool_max * 192 // 1024} MB RAM)")
            else:
                print(f"[dataset] loading all {len(files_to_load)} clean files to RAM "
                      f"(~{len(files_to_load) * 192 // 1024} MB)")
            for p in files_to_load:
                try:
                    c = self._load(p)
                    if c.size > 0:
                        self.clean_pool.append(c)
                except Exception:
                    continue
            print(f"[dataset] clean pool ready: {len(self.clean_pool)} files in RAM")
    ...
    # ------------------------------------------------------------------
    # LF / HF noise grabbers (training 70/30 split)
    # ------------------------------------------------------------------

    def _get_pool_noise(self, length: int, rng,
                        pool: Optional[List[np.ndarray]] = None) -> np.ndarray:
        """Pull a length-exact segment from the given pool (default: noise_pool)."""
    ...
    def _grab_lf_noise(self, length: int) -> np.ndarray:
        """Real LF-dominant noise from real_lf_pool, blended with synthetic LF."""
    ...
    def _grab_hf_noise(self, length: int) -> np.ndarray:
        """General/HF noise from general_noise_pool, blended with synthetic HF archetypes."""
        archetype = random.choice(["click", "hiss", "burst"])
        if archetype == "click":
            noise = generate_click_noise(length, self.sample_rate)
        elif archetype == "hiss":
            noise = generate_hiss_noise(length, self.sample_rate)
        else:
            noise = generate_burst_noise(length, self.sample_rate)
        if self.general_noise_pool and random.random() < 0.6:
            ...
            noise = blend * pool_seg + (1.0 - blend) * noise
        rms = _rms(noise)
        return (noise / (rms + EPS)).astype(np.float32)
    ...
    def __getitem__(self, idx: int):
        ...
        # Training: sample randomly from RAM pool (no disk I/O) if pool is available.
        # Val/test: always load by index for deterministic evaluation.
        if self.augment and self.clean_pool:
            clean = random.choice(self.clean_pool).copy()
        else:
        ...
        if self.augment:
            ...
            # Always mix 2 real noises: 1 LF (real_lf_pool) + 1 general
            ...
            # bandpass_filter removed: applying it only to noisy (not clean) creates an
            # impossible reconstruction target — the model is penalised for not outputting
            # LF content that was never present in its input. Contradictory gradients
            # cause a loss plateau from epoch 1.
            # freq_mask_waveform likewise removed for the same reason.
        else:
            ...
            noise = self._grab_noise(self.segment_len, rng=rng)
