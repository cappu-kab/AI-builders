...
clean utterance.  This is the simplest way to drive a denoiser toward
...
# Defaults
# ---------------------------------------------------------------------------
SAMPLE_RATE = 16_000          # if you preprocessed at a different sr, change here
SEGMENT_SECONDS = 4.0         # fixed crop length used for batching
...
def mix_at_snr(clean: np.ndarray, noise: np.ndarray, snr_db: float) -> Tuple[np.ndarray, float]:
    ...
    # alpha so that 20*log10(clean_rms / (alpha*noise_rms)) == snr_db
...
def pitch_shift(x: np.ndarray, p: float = 0.3,
    ...
    # speed up by `rate` (changes pitch+duration), then time-stretch back to length
...
def freq_mask_waveform(x: np.ndarray, p: float = 0.3,
    ...
    y = torch.istft(spec, n_fft=n_fft, hop_length=hop,
                    window=torch.hann_window(n_fft), length=x.size)
...
def apply_rir(x: np.ndarray, p: float = 0.4,
    ...
    # -60 dB at decay_samples  →  exponent = -13.816
    rir = np.exp(-13.816 * t / decay_samples)
    # scatter a few early reflections
    ...
    for _ in range(n_ref):
        ...
        if delay < decay_samples:
            ...
            rir[delay] += amp
    rir /= (np.abs(rir).sum() + EPS)          # unit-energy impulse response

    # FFT-based linear convolution, truncated back to original length
    n_fft = len(x) + len(rir) - 1
    Y     = np.fft.rfft(x,   n=n_fft) * np.fft.rfft(rir, n=n_fft)
    y     = np.fft.irfft(Y)[:len(x)].astype(np.float32)

    # preserve RMS so downstream SNR logic is unaffected
...
def random_eq(x: np.ndarray, p: float = 0.4,
    ...
    frequency notches.  Gain is capped at ±3 dB per band with 2-3 bands so the
    worst-case combined attenuation across any frequency is ≈ −9 dB — audible
    coloration but not the deep notches (≥ −30 dB) that create musical noise.
    Phase-preserving (FFT multiply), RMS-normalised after filtering.
    """
    if random.random() > p or x.size < 4:
        return x
    X      = np.fft.rfft(x)
...
def soft_clip(x: np.ndarray, p: float = 0.25) -> np.ndarray:
    ...
    if random.random() > p:
        return x
    drive     = random.uniform(1.5, 5.0)      # higher = more compression / distortion
    rms_in    = float(np.sqrt(np.mean(x ** 2) + EPS))
...
def _read_labels(csv_path: Path) -> Dict[str, str]:
    ...
    with open(csv_path, "r", encoding="utf-8", newline="") as fh:
        ...
        if fname_col is None:
            # No recognised header — positional fallback (col 0 → col 1)
            fh.seek(0)
            for i, row in enumerate(csv.reader(fh)):
                if not row or i == 0:   # always skip first row (assumed header)
                    continue
                fname = row[0].strip()
...
class SpeechDenoiseDataset(Dataset):
    ...
    def _load(self, path: Path) -> np.ndarray:
        ...
        if x.ndim > 1:               # stereo -> mono
            x = x.mean(axis=0)
        # de-mean to remove DC offsets that wreck SNR computations
        x = x - x.mean()
        return x
    ...
    def _grab_noise(self, length: int) -> np.ndarray:
        ...
        Using multiple sources per sample creates ~3× more distinct noise
        textures from the same noise pool, addressing data-diversity constraints
        ...
        for _ in range(n_sources):
            # build one noise segment by concatenating files until long enough
            ...
            # random relative weight [0.3, 1.0] before mix_at_snr normalises overall level
            weight = random.uniform(0.3, 1.0) if n_sources > 1 else 1.0
            combined += weight * segment

        # Renormalise so mix_at_snr's RMS calculation stays meaningful
