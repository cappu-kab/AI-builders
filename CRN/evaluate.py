...
def _score_wer_cer(ref: str, hyp: str) -> Tuple[float, float]:
    """Return *(WER, CER)* for a reference / hypothesis pair.

    WER
    ---
    Both strings are first tokenised with ``_tok_thai`` (pythainlp when
    available, else whitespace-split), then space-joined, and finally scored
    with ``jiwer.wer``.  This correctly handles Thai text that has no natural
    word boundaries.

    CER
    ---
    ``jiwer.cer`` on the stripped raw strings (character-level).  For Thai, CER
    is often more informative than WER because a mis-segmentation by the ASR
    model does not necessarily mean a character is wrong.
    """
    ...
    if _HAVE_JIWER:
        wer = float(_jiwer.wer(ref_str, hyp_str))
        cer = float(_jiwer.cer(ref.strip(), hyp.strip()))
    else:
        if not ref_toks:
            wer = 0.0 if not hyp_toks else 1.0
        else:
            wer = _edit_distance(ref_toks, hyp_toks) / len(ref_toks)
